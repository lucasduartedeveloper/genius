...
$(document).ready(function() {
    ...
    elem.onclick = function() {
        ...
        document.body.appendChild(elem20);
    };
    ...
    elem8.onclick = function() {
        ...
        if (nextObj.countRound) {
            ...
            var text = 
            finger_name[nextObj.finger_no][language_no == 0 ? 1 : 0] + " ";
            if (direction == 0) 
            text += letters[language_no == 0 ? 1 : 0]
            [nextObj.px]+(nextObj.py+1);
            else 
    ...
    elem10 = document.createElement("span");
    elem10.style.position = "absolute";
    elem10.className = "animate__animated";
    elem10.innerText = "Fácil";
    elem10.style.color = "#fff";
    elem10.style.textAlign = "center";
    elem10.style.left = ((sw/2)-(変数/2))+"px";
    elem10.style.top = ((sh/2)-(変数/2)+(sw/9))+"px";
    elem10.style.width = (変数)+"px";
    elem10.style.height = (sw/9)+"px";
    ...
    elem10.onclick = function() {
        ...
        elem10.classList.remove("animate__slideInLeft");
        elem10.classList.add("animate__slideOutRight");
        if (difficulty == 0) elem10.innerText = "Fácil";
        else if (difficulty == 1) elem10.innerText = "Médio";
        else elem10.innerText = "Lucas";
    };
    document.body.appendChild(elem10);

    elem10.addEventListener("animationend", function() {
        elem10.classList.remove("animate__slideOutRight");
        elem10.classList.add("animate__slideInLeft");
    });
    ...
    elem13.style.backgroundSize = "cover";
    ...
    elem13.onclick = function() {
       var rnd = Math.floor(Math.random()*5);
       nextObj.finger_no = rnd;
       drawHand(true, rnd);
    };
    drawHand(true, nextObj.finger_no);
    document.body.appendChild(elem13);

    elem15 = document.createElement("span");
    elem15.style.position = "absolute"
    elem15.style.background = "#fff";
    elem15.style.opacity = "0.5";
    elem15.style.color = "black";
    elem15.style.fontWeight = "500";
    elem15.style.fontSize = (sw/18)+"px";
    elem15.style.lineHeight = (変数/4)+"px";
    elem15.style.textAlign = "center";
    elem15.style.left = (((sw-sw)/2)+(sw/2)-(sw/2))+"px";
    elem15.style.top = ((sh/2)-(sw/2)-(sw/9))+"px";
    elem15.style.width = (sw)+"px";
    elem15.style.height = (変数/2)+"px";
    //elem15.style.border = "2px solid black";
    elem15.style.borderRadius = (変数/16)+"px";
    /*beforeAudio = function(text) {
        elem15.innerText = text;
    ...
    elem17 = document.createElement("span");
    elem17.style.position = "absolute";
    elem17.innerHTML = drawText(nextObj);
    elem17.style.color = "#99ff99";
    elem17.style.fontSize = (sw/18)+"px";
    elem17.style.textAlign = "center";
    elem17.style.left = ((sw/2)-(変数/4))+"px";
    elem17.style.top = ((sh/2)-(変数/2)-(sw/9))+"px";
    elem17.style.width = (変数/2)+"px";
    elem17.style.height = (sw/9)+"px";
    document.body.appendChild(elem17);
    ...
    elem18 = document.createElement("span");
    elem18.style.position = "absolute";
    elem18.className = "animate__animated";
    elem18.style.background = "orange";
    elem18.style.left = ((sw/2)-(変数/2))+"px";
    elem18.style.top = ((sh/2)+(変数/2)+(sw/40))+"px";
    elem18.style.width = (sw/40)+"px";
    elem18.style.height = (sw/40)+"px";
    elem18.style.borderRadius = "50%";
    document.body.appendChild(elem18);

    elem18.addEventListener("animationend", function() {
        elem18.classList.remove("animate__slideOutRight");
    });

    elem19 = document.createElement("span");
    elem19.style.position = "absolute";
    elem19.className = "animate__animated";
    elem19.style.background = "purple";
    elem19.style.right = ((sw/2)-(変数/2))+"px";
    elem19.style.top = ((sh/2)+(変数/2)+(sw/40))+"px";
    elem19.style.width = (sw/40)+"px";
    elem19.style.height = (sw/40)+"px";
    elem19.style.borderRadius = "50%";
    document.body.appendChild(elem19);

    elem20 = document.createElement("span");
    elem20.style.position = "absolute";
    elem20.className = "animate__animated";
    //elem20.style.background = "#fff";
    elem20.style.right = ((sw/2)-(sw/60))+"px";
    elem20.style.top = ((sh/2)+(変数/2)+(sw/40))+"px";
    elem20.style.width = (sw/30)+"px";
    elem20.style.height = (sw/30)+"px";
    elem20.style.border = "2px solid white";
    elem20.style.borderRadius = "50%";
    //document.body.appendChild(elem20);

    audioBot = true;
    elem21 = document.createElement("i");
    elem21.style.position = "absolute";
    elem21.style.color = "#fff";
    elem21.className = "fa-solid fa-volume-high";
    elem21.style.fontSize = (sw/20);
    elem21.style.left = ((sw/2)-(変数/2)+5)+"px";
    elem21.style.top = ((sw/2)-(変数/2)+(変数/4)+5)+"px";
    elem21.style.width = (sw/20)+"px";
    elem21.style.height = (sw/20)+"px";
    elem21.onclick = function() {
        audioBot = !audioBot;
        elem21.className = audioBot ? 
        "fa-solid fa-volume-high" :
        "fa-solid fa-volume-xmark";
    };
    document.body.appendChild(elem21);

    elem19.addEventListener("animationend", function() {
        elem19.classList.remove("animate__slideOutLeft");
    });

    /*nextObj = next(false, true);
    triangle.style.backgroundImage = draw_(nextObj.cx, nextObj.cy);
    elem17.innerHTML = drawText();*/
...
var getAvatar = function() {
    var rnd = Math.random();
    var n = Math.floor(Math.random()*avatar_addr.length);
    return avatar_addr[n];
};
...
var drawText = function(nextObj) {
    ...
    if (nextObj.countRound) {
        ...
        switch(nextObj.direction) {
            case 0:
                if (direction == 0) 
                text += letters[language_no == 0 ? 1 : 0][0][0]+"1";
                else 
                text += letters[language_no == 0 ? 1 : 0][0][0]+"1";
                break;
            case 1:
                if (direction == 0) 
                text += letters[language_no == 0 ? 1 : 0][7][0]+"1";
                else 
                text += letters[language_no == 0 ? 1 : 0][0][0]+"8";
                break;
            case 2:
                if (direction == 0) 
                text += letters[language_no == 0 ? 1 : 0][7][0]+"8";
                else 
                text += letters[language_no == 0 ? 1 : 0][7][0]+"8";
                break;
            case 3:
        ...
        if (direction == 0) 
        text += letters[language_no == 0 ? 1 : 0]
        [nextObj.px][0]+(nextObj.py+1);
        else 
        text += letters[language_no == 0 ? 1 : 0]
        [nextObj.py][0]+(nextObj.px+1);
        html = text + 
        "&nbsp;<i class=\"fa-solid fa-arrow-right\"></i>&nbsp;";
        text = "";
        if (direction == 0) 
        text += letters[language_no == 0 ? 1 : 0]
        [nextObj.px2][0]+(nextObj.py2+1);
        else 
        text += letters[language_no == 0 ? 1 : 0]
        [nextObj.py2][0]+(nextObj.px2+1);
        html += text;
    }
    else if (nextObj.dirRound) {
        switch(nextObj.direction) {
        ...
        if (!invert)
        html = colors[color_no][language_no == 0 ? 1 : 0];
        else
        html = colors[color_no == 0 ? 1 : 0][language_no == 0 ? 1 : 0];
    }
    else {
        var text = "";
        if (direction == 0) 
...
var loadHs = function() {
    if (location.href.includes("192")) {
         var local_hs = localStorage.getItem("hs");
         if (local_hs) hs = local_hs;
         elem4.innerText = "HIGHSCORE: x"+hs;
         return;
    }
...
var draw = 
    ...
    if (nextObj.countRound && count == nextObj.count) {
        ...
        jumpRate = 5;
        if (!loading) {
        $(elem18).animate({ left: "+="+(変数) }, 1000, function() {
            // Animation complete.
            $(elem18).animate({ left: "-="+(変数) }, 1000);
        });
        $(elem19).animate({ left: "-="+(変数) }, 1000, function() {
            // Animation complete.
            $(elem19).animate({ left: "+="+(変数) }, 1000);
        });
        if (lives > 1 && typeof(elem8) != "undefined") { 
            beepPool.play("audio/mario-die.wav");
            lives -= 1; elem8.innerHTML = drawLives(); }
        else { 
            lives -= 1; elem8.innerHTML = drawLives();
            triangle.remove();
            playingTime = new Date().getTime() - playingTime
            elem10.onclick = null;
            var text = "";
            if (language_no == 0) {
            text = 
            "<span style=\"color:purple\">" +
            stack.length + " acertos em " +
            (playingTime/1000).toFixed(3).replace(".",",") +
            " segundos. </span>";
            elem10.innerHTML = "GAME OVER<br>"+ text +
            "<br><br> Tempo médio de interpretação: <br>" +
            "<span style=\"color:orange\">" +
            ((playingTime/1000)/stack.length).toFixed(3).replace(".",",") + 
            " segundos</span>";
            text = 
            stack.length + " acertos em " +
            parseInt((playingTime/1000))+
            " segundos.";
            }
            else {
            text = 
            "<span style=\"color:purple\">" +
            stack.length + " hits in " +
            (playingTime/1000).toFixed(3).replace(".",",") +
            " seconds.</span>";
            elem10.innerHTML = "GAME OVER<br>"+ text +
            "<br><br> Average interpretation time: <br>" +
            "<span style=\"color:orange\">" +
            ((playingTime/1000)/stack.length).toFixed(3).replace(".",",") + 
            " seconds</span>";
            text = 
            ...
            var local_avg_time = localStorage.getItem("avg_time");
            var positive_time = false;
            if (local_avg_time) {
                positive_time = (playingTime/1000) < local_avg_time;
                if (language_no == 0) {
                    text += (playingTime/1000) < local_avg_time ?
                    " Você interpretou mais rápido " :
            ...
            localStorage.setItem("avg_time", (playingTime/1000));

            var local_last_score = localStorage.getItem("last_score");
            if (local_last_score) {
                if (language_no == 0) {
                    text += stack.length > local_last_score ?
                    (!positive_time ? "porém" : "e") + " acertou mais." :
                    (positive_time ? "porém" : "e") + " acertou menos.";
                }
                else {
                    text += stack.length > local_last_score ?
                    (!positive_time ? "but" : "and") + " answered more." :
                    (positive_time ? "but" : "and") + " answered less.";
                }
            }
            localStorage.setItem("last_score", stack.length);
    ...
    if (!loading) {
    var ballSize = (sw/40)+(stack.length*0.1);
    elem18.style.width = ballSize+"px";
    elem18.style.height = ballSize+"px";
    elem19.style.width = ballSize+"px";
    elem19.style.height = ballSize+"px";
    }
...
var drawBot = function(d) {
    ...
    // Create path (x)
    ctx.beginPath();
    ctx.moveTo(10, (変数/8)-10);
    ctx.lineTo((変数/8)-10, 10);

    ctx.moveTo((変数/8)-10, (変数/8)-10);
    ctx.lineTo(10, 10);
    ctx.stroke();
...
var finger_name = [
    [ "little finger to", "dedinho no" ],
    [ "ring finger to", "anelar no" ],
    [ "middle finger to", "dedo médio no" ],
    [ "index finger to", "indicador no" ],
    [ "thumb to", "polegar no" ]
];
...
var next = function(cancolor=true, nojump=false) {
    ...
    var obj = {
        memoryRound: difficulty >= 2 &&
        cancolor, // && Math.floor(Math.random()*5) == 0,
        countRound: difficulty >= 1 &&
        cancolor && Math.floor(Math.random()*5) == 0,
        botRound: difficulty >= 2 &&
        cancolor && Math.floor(Math.random()*5) == 0,
        connectRound: difficulty >= 1 &&
        cancolor && Math.floor(Math.random()*3) == 0,
        dirRound: difficulty >= 1 &&
        cancolor && Math.floor(Math.random()*3) == 0,
        colorRound: difficulty >= 0 &&
        cancolor && Math.floor(Math.random()*3) == 0,
        count: 
        ...
        px3: Math.floor(Math.random()*8),
        py3: Math.floor(Math.random()*8),
        bot: { running: false, mov: 1 },
        finger_no: 3, //Math.floor(Math.random()*5)
    };
    obj.cx = (obj.px*(変数/8))+(変数/16);
    obj.cy = (obj.py*(変数/8))+(変数/16);
    obj.cx2 = (obj.px2*(変数/8))+(変数/16);
    obj.cy2 = (obj.py2*(変数/8))+(変数/16);
    obj.cx3 = (obj.px3*(変数/8))+(変数/16);
    obj.cy3 = (obj.py3*(変数/8))+(変数/16);
    ...
    if (obj.countRound) {
        ...
        obj.finger_no = Math.floor(Math.random()*5);
        var text = 
        finger_name[obj.finger_no][language_no == 0 ? 1 : 0] + " ";
        if (direction == 0) 
        text += letters[language_no == 0 ? 1 : 0][obj.px]+(obj.py+1);
        else 
        text += letters[language_no == 0 ? 1 : 0][obj.py]+(obj.px+1);
    ...
    if (!loading) drawHand(true, obj.finger_no);
    if (!loading) elem17.innerHTML = drawText(obj); 
    return obj;
};

var finger_pos = [
   { x: (1/1077)*321, y: (1/1077)*210 },
   { x: (1/1077)*431, y: (1/1077)*125 },
   { x: (1/1077)*545, y: (1/1077)*75 },
   { x: (1/1077)*655, y: (1/1077)*75 },
   { x: (1/1077)*861, y: (1/1077)*441 }
];

var drawHand = function(update=false, finger_no=0) {
    ...
    img.onload = function() {
        var ctx = this.ctx;
        ctx.drawImage(this, 0, 10, (変数/2), (変数/2));
        ctx.beginPath();
        ctx.arc(this.pos.x*(変数/2), this.pos.y*(変数/2), 
        変数/40, 0, 2*Math.PI);
        ctx.fill();
...
var forceMistakes = false;
var forcedMistakeRate = 20;

var skip = function() {
    var rnd = Math.floor(Math.random()*forcedMistakeRate);
    if (forceMistakes && rnd == 0) {
        draw(-1, -1);
    }
    else if (nextObj.countRound) {
