...
var getHorizontalConnectionPoints = function(points, radius) {
    var connections = [];
    for (var n = 0; n < (points.length-1); n++) {
            var c0 = { x: points[n].x, y: points[n].y };
            var p0 = { x: points[n].x+radius, y: points[n].y };

            var c1 = { x: points[n+1].x, y: points[n+1].y };
            var p1 = { x: points[n+1].x-radius, y: points[n+1].y };
